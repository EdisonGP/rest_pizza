...
const getPizzas =async (req,res)=>{
    const response = await db.any('select * from pizza where piz_state=true')
    res.json(response)
}
...
const postCreatePizza =async (req,res)=>{
    ...
    const response = await db.any(`INSERT INTO pizza (piz_name,piz_origin,piz_state) values($1,$2,$3)`,[piz_name,piz_origin,piz_state])
    res.json({
...
const putUpdatePizza =async (req,res)=>{
    ...
    const response = await db.any(`UPDATE pizza set piz_name=$2, piz_origin=$3, piz_state=$4 where piz_id=$1`,[piz_id, piz_name,piz_origin,piz_state])
    res.json({
...
const postCreateIngredients =async (req,res)=>{
    ...
    const response = await db.any(`INSERT INTO ingredient (ing_name,ing_calories,ing_state) 
...
const putUpdateIngredients =async (req,res)=>{
    ...
    const response = await db.any(`UPDATE ingredient set ing_name=$2, ing_calories=$3 where ing_id=$1`,[ing_id,ing_name,ing_calories])
    res.json({
        ...
        { 
            ...
            for (let i = 0; i < pizzas.length; i++) { 
                ...
                const ingresdientPizza = await db.any(`select i.ing_id, i.ing_name, i.ing_calories 
                                                ...
                                                ingredient i where  p.piz_id=pi.piz_id 
...
const postCreatePizzaIngredients =async (req,res)=>{
    ...
    const response = await db.any(`INSERT INTO pizza_ingredient (piz_id,ing_id,piz_ing_state)  values($1,$2,true)`,[piz_id,ing_id])
    res.json({
...
const getPizzasIngredientsByPizzaId =async (req,res)=>{
    ...
    from pizza_ingredient pi, pizza p, ingredient i where pi.piz_id=p.piz_id and pi.ing_id=i.ing_id 
